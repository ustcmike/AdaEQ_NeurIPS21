...
def redq_sac(env_name, seed=0, epochs='mbpo', steps_per_epoch=1000,
    ...
    for t in range(total_steps):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            # if epoch % 10 == 0 and q_target_mode == 'min':
            #   if epoch_exp_error > threshold and agent.num_min < 10:  
            #       lower_bound = agent.num_min + 1           
            #       agent.num_min = np.random.randint(lower_bound, 11) 
            #   elif epoch_exp_error < threshold and agent.num_min > 3:
            #       upper_bound = agent.num_min 
            #       agent.num_min = np.random.randint(2, upper_bound)
            #   else :
            #     #agent.num_min = np.random.randint(2, 3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='non_ada_sac')
    ...
    parser.add_argument('--num_Q', '-n', type=int, default=10)
    ...
    exp_name_full = args.exp_name + '_%s_' % args.env + 't_%s' % args.threshold + '_M_%s' % args.num_Q + '_%s' % args.q_target_mode
