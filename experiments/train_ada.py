...
def redq_sac(env_name, seed=0, epochs='mbpo', steps_per_epoch=1000,
             ...
             policy_update_delay=20, parameterc = 0.4, threshold = 0.4
             ):
    """
    ...
    for t in range(total_steps):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            #n_eval = 1 (only one episode used to get the value)
            if epoch % 10 == 0:
              if  epoch_std_error > parameterc and agent.num_min < 10:                
                  lower_bound = agent.num_min + 1           
                  agent.num_min = np.random.randint(lower_bound, 11) 
              elif epoch_std_error < parameterc and agent.num_min > 3:
                  upper_bound = agent.num_min 
                  agent.num_min = np.random.randint(2, upper_bound)
              else :
                  agent.num_min = agent.num_min
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='ada_sac')
    parser.add_argument('--data_dir', type=str, default='../old_data_real_adaptive/')
    parser.add_argument('--debug', action='store_true')
    parser.add_argument('--parameterc', '-c', type=float, default=0.4)
    ...
    exp_name_full = args.exp_name + '_%s' % args.env + '_%s' % args.parameterc+ '_%s' % args.threshold
    ...
    redq_sac(args.env, seed=args.seed, epochs=args.epochs,
             logger_kwargs=logger_kwargs, debug=args.debug, parameterc=args.parameterc, threshold = args.threshold)
